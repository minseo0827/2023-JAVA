 ...
 * 
 * class MyFrame1 extends JFrame { JTextField id, title, p, year, price, author;
 * JButton previousButton, nextButton, InsertButton, deleteButton, searchButton;
 * ResultSet rs; Statement stmt;
 * 
 * public MyFrame1() throws SQLException { super("Database Viewer"); Connection
 * con = makeConnection(); stmt =
 ...
 * 
 * add(new JLabel("PUBLISHER", JLabel.CENTER)); add(p = new JTextField());
 * 
 * add(new JLabel("YEAR", JLabel.CENTER)); add(year = new JTextField());
 * 
 * add(new JLabel("PRICE", JLabel.CENTER)); add(price = new JTextField());
 * 
 * add(new JLabel("저자검색", JLabel.CENTER)); add(author = new JTextField());
 * 
 ...
 * p.setText("" + rs.getString("publisher")); year.setText("" +
 ...
 * rs.getString("title")); p.setText("" + rs.getString("publisher"));
 * year.setText("" + rs.getString("year")); price.setText("" +
 * rs.getInt("price"));
 * 
 * } catch (SQLException e) { e.printStackTrace(); } } });
 * 
 * InsertButton = new JButton("Insert"); deleteButton = new JButton("Delete");
