 ...
 * 
 * // 배경 이미지(bg.jpg) 추가 //JLabel bgLabel = new JLabel(); //bgLabel.setIcon(new
 * ImageIcon("bg.png")); //bgLabel.setBounds(0, 0, 800, 400); //add(bgLabel);
 * getContentPane().setBackground(java.awt.Color.GRAY);
 * 
 * // 다섯 대의 차량 생성 및 스레드 시작 (new MyThread("car1", 100, 0)).start(); (new
 * MyThread("car2", 100, 50)).start(); (new MyThread("car3", 100, 100)).start();
 * (new MyThread("car4", 100, 150)).start(); (new MyThread("car5", 100,
 * 200)).start();
 * 
 ...
 * setVisible(true); }
 * 
 * private void displayWinsMessage(String carFileName) { JLabel winsLabel = new
 * JLabel(carFileName + "이 우승했습니다!"); winsLabel.setBounds(550, 300, 150, 100);
