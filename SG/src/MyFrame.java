...
class MyFrame extends JFrame {
	...
	public MyFrame() throws SQLException {
		...
		InsertButton.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent event) {
		        try {
		            ...
		            String publisherValue = publisher.getText();  // 수정: 출판사 변수명 변경
		            String bookYear = year.getText();
		            int bookPrice = Integer.parseInt(price.getText());

		            // 데이터베이스에 새로운 레코드 삽입
		            String insertQuery = String.format("INSERT INTO books VALUES (%d, '%s', '%s', '%s', %d)",
		                    bookId, bookTitle, publisherValue, bookYear, bookPrice);
		...
		deleteButton.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent event) {
		        try {
		            // 검색 결과에 따라 다르게 현재 행의 book_id를 가져옴
		            int currentBookId = (rs != null && !rs.isClosed()) ? rs.getInt("book_id") : Integer.parseInt(id.getText());

		            // 데이터베이스에서 현재 행 삭제
		            String deleteQuery = String.format("DELETE FROM books WHERE book_id = %d", currentBookId);
		            stmt.executeUpdate(deleteQuery);

		            // 삭제 후에는 결과셋을 다시 가져와야 함
		            rs = stmt.executeQuery("SELECT * FROM books");
		            ...
		            if (rs.first()) {
		                ...
		                // 결과셋이 비어있으면 GUI를 초기화하거나 사용자에게 메시지 출력
		                id.setText("");
		                title.setText("");
		                publisher.setText("");
		                year.setText("");
		...
		ClearButton.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent event) {
		        ...
		        year.setText("");
		        price.setText("");
		        search.setText(""); // 추가: search 필드도 비움
		    }
		});
